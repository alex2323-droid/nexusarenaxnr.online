...
// Also remove the placeholder motion.div
const placeholderRegex = /<motion\.div\s+initial=\{\{ opacity: 0, y: 20 \}\}\s+animate=\{\{ opacity: 1, y: 0 \}\}\s+transition=\{\{ delay: 0\.2 \}\}\s+className="flex items-center justify-between p-6 bg-yellow-500\/5 border border-yellow-500\/10 rounded-3xl"\s*>[\s\S]*?<\/motion\.div>\s+<!-- New Achievements Section -->/g;

// I will do it with string replacement to be safer.
